...
def crawl_web(seed, max_debth):
    ...
    while to_crawl: 
        ...
        page, distance = to_crawl.pop()
        if page not in crawled and distance<=max_debth:
            #proccess page
            links = get_all_links (getpage(page))
            union(to_crawl, add_distance (links, distance+1))
            crawled.append(page)

    return crawled

def add_distance(links,distance):
    result = []
    for i in links:
